...
export default function TopManga() {
  ...
  const getTopManga = async () => {
      const response = await fetch(process.env.NEXT_PUBLIC_API_ENDPOINT+`manga/top?page=${pageNumber}`);
      return response.json();   
  };
    ...
    getTopManga()
      .then((res) => {
        ...
        medias.forEach((media,index) => {
            setTopManga(topManga => [...topManga, media])
        })
      })
      .catch((e) => {
    ...
    <>
      ...
      <Container>
        <main className="pb-10 2xl:px-28 xl:px-16  lg:px-2 sm:px-4 px-4">
            <h1 className="text-accent font-semibold ml-3 text-2xl mt-10">Top 100 Manga</h1>
          <section id="filters">
